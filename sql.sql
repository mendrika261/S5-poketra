...
SELECT
    "idPersonnel" AS id_personnel,
    "nom",
    "nomService" AS nom_service,
    s."prixHoraire" AS prix_horaire_base,
    floor(extract(day FROM (now() - "dateAffectation")) / 365) AS anciennete,
...
SELECT
    nom AS nom_poste,
    coefficient AS coeff_poste,
    id id_poste
FROM
    "Poste"
    JOIN (
        SELECT
            min(abs(5 - anciennete)) AS poste_anciennete
        FROM
            "Poste" p
        ORDER BY
            poste_anciennete
        LIMIT 1) AS min_poste ON anciennete = 5 - poste_anciennete
